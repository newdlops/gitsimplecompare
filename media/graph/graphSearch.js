...
(function () {
  ...
  let latestRequestId = "";
  let pendingJump;
  let repoState = { status: "idle", query: "", matches: [], skippedCommitSearch: false };
  ...
  function startSearch(input, results) {
    ...
    if (!query) {
      latestRequestId = "";
      repoState = { status: "idle", query: "", matches: [], skippedCommitSearch: false };
      render(input, results);
      return;
    }
    latestRequestId = nextRequestId("search");
    repoState = { status: "loading", query, matches: [], skippedCommitSearch: false };
    render(input, results);
    debounceTimer = window.setTimeout(() => {
      window.GscGraphPostMessage?.({ type: "graphRepositorySearch", requestId: latestRequestId, query });
    }, DEBOUNCE_MS);
  }
  ...
  function fetchRemoteRefs(input, results) {
    const query = input.value.trim();
    if (!query) {
      return;
    }
    ...
    repoState = { status: "loading", query, matches: [], skippedCommitSearch: false, fetching: true };
    render(input, results);
    window.GscGraphPostMessage?.({ type: "fetchGraphSearchRefs", requestId: latestRequestId, query });
  }
  ...
  function render(input, results) {
    ...
    const loaded = localCandidates(terms).slice(0, 30);
    results.hidden = false;
    results.innerHTML =
      sectionHtml("Loaded matches", loaded, "No matches in loaded commits") +
      repositorySection(query) +
      footerHtml();
  }

  /** 현재 로드된 row 에서 검색 후보를 만든다. */
  function localCandidates(terms) {
    if (!rootEl) {
      return [];
    }
  ...
  function repositorySection(query) {
    if (repoState.query !== query || repoState.status === "idle") {
  ...
  function footerHtml() {
    const title = "Fetch remote refs, then search again";
    return `<div class="search-footer"><span>Repository search uses local refs.</span>` +
      `<button type="button" data-search-fetch title="${esc(title)}" aria-label="${esc(title)}" data-tooltip="${esc(title)}">` +
      `<span class="codicon codicon-repo-fetch" aria-hidden="true"></span><span>Fetch remote refs</span></button></div>`;
  }
  ...
  window.addEventListener("message", (event) => {
    ...
    if (msg.type === "graphRepositorySearchResult" && msg.requestId === latestRequestId) {
      repoState = Object.assign({ status: "ready" }, msg.result || {});
      update();
    } else if (msg.type === "graphRepositorySearchError" && msg.requestId === latestRequestId) {
      repoState = { status: "error", query: msg.query || "", matches: [], message: msg.message };
      update();
