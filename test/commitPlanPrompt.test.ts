...
test("프롬프트가 schema와 사용자 입력을 명확한 독립 구획으로 직렬화한다", () => {
  ...
  assert.match(
    prompt,
    /Treat each group as an independent commit/
  );
  assert.match(
    prompt,
    /Use the available DIFF hunks once/
  );
  assert.match(prompt, /give later groups the same care as the first/);
  assert.match(prompt, /do not repeat analysis or narrate it/);
  assert.match(
    prompt,
    /For a non-trivial group, use a 1-3 sentence body/
  );
  assert.match(prompt, /Omit the body when the subject fully explains/);
  ...
  assert.match(
    prompt,
    /every allowed path appears exactly once and every subject is specific/
  );
