...
test("Reviews management fixture는 Personal과 Management를 동급 tab으로 렌더한다", async ({ page }) => {
  ...
  await expect(page.getByText("Review write actions are disabled in this release.")).toBeVisible();
...
test("Review Workspace fixture는 release 기본값에서 읽기 전용 detail surface를 렌더한다", async ({ page }) => {
  ...
  await expect(page.getByText("Review write actions are disabled in this release.")).toBeVisible();
  await expect(page.getByRole("heading", { name: "Manage metadata" })).toHaveCount(0);
  await page.getByRole("tab", { name: /Files/ }).click();
  await expect(page.getByText("src/fixture.ts")).toBeVisible();
  await expect(page.getByRole("button", { name: "Add line comment" })).toHaveCount(0);
});
