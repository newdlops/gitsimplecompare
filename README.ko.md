...
**Changes → … → 로컬 브랜치 현황 및 Stale 정리…**를 누르거나 명령 팔레트에서 **Git Simple Compare: 로컬 브랜치 현황 및 Stale 정리…**를 실행합니다. 전체 로컬 브랜치를 로컬 이름 기준으로 표시하며, stale 여부·대응 원격·worktree 보호 사유·최신 커밋을 함께 확인할 수 있습니다. 검색과 **Stale만 보기**로 정리할 항목을 찾습니다. 원격에 있거나 사용 중인 브랜치도 목록에 남으며 선택 컨트롤만 비활성화됩니다.

등록된 모든 원격을 직접 조회하므로 fetch하지 않은 브랜치와 오래된 원격 추적 정보도 정확히 구분합니다. Stale 브랜치는 어떤 원격에도 같은 이름이 없는 로컬 브랜치이며, 생성 시점이나 upstream 설정은 판별 기준이 아닙니다. 등록된 원격이 없어도 로컬 목록은 표시하며 상태는 **미확인**으로 안내합니다.

정리할 stale 브랜치를 선택한 뒤 **삭제 검토**를 눌러 이름을 확인하고 삭제합니다. 선택한 항목이 검색이나 필터로 숨겨져 있으면 그 수를 별도로 표시합니다. 현재 또는 다른 worktree에서 체크아웃한 브랜치는 보호하며, 미병합 브랜치는 **강제 삭제**를 별도로 확인합니다. 삭제 전에 원격 연결과 설정, 브랜치 커밋, worktree 사용 여부를 다시 검사하고 실패나 상태 변경은 **Git Simple Compare** OUTPUT에 기록합니다. 원격 브랜치와 작업 파일은 보존합니다.
