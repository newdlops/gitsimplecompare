...
7. **버튼 툴팁**: UI 에 버튼이나 버튼처럼 동작하는 컨트롤(`button`, `role="button"`, 아이콘 액션, hover-only 액션)을 만들 때는 항상 hover tooltip 을 둔다. 웹뷰 버튼은 `title` 또는 즉시 뜨는 `data-tooltip` 과 접근성 라벨을 함께 제공하고, VS Code 기여 명령 버튼은 명확한 command title 을 제공한다.
