...
- Manual VSIX: download or build a `.vsix`, then run `code --install-extension gitsimplecompare-0.1.72048.vsix`
...
If a planned commit fails, its numbered failure details include a **Commit log** preview. Use **Hide log** / **Show log** to collapse or expand it, and **Copy log** to copy the complete captured stdout/stderr even when the preview is shortened. Copy results appear beside the log; a failed copy leaves the output available for retry.
