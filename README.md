...
6. **Context-aware conflict resolution** — inspect the exact Current/Incoming commit sources, editable Result, and rebase todo impact before choosing a side; the native merge editor remains available as a fallback.
...
During a merge/rebase/cherry-pick/revert, the **Conflicts** view opens a context-aware editor. It identifies the commits behind Current and Incoming, keeps Base available for reference, and makes only Result editable. During a rebase it also shows the original branch tip, new base, active todo step, remaining commits that touch the same path, and whether later history can change the selected Result. Current/Ours is the accumulated rebase side; Incoming/Theirs is normally the replayed commit and is labeled from the active `MERGE_HEAD`/`CHERRY_PICK_HEAD` instead when a nested operation causes the conflict. The one-click Current/Incoming actions replace the whole file, while block actions change only an individual conflict-marker block. The native 3-way merge editor remains available from the toolbar. Use **Continue** / **Abort** in the Conflicts view after resolving the files.
