...
## Local Branch Status and Stale Cleanup

- 정리 화면의 행 기준은 `refs/heads`의 로컬 브랜치 이름이다. 원격에만 있는 브랜치는 행으로 표시하지 않는다.
- 상단에는 저장소와 로컬 전체·stale·삭제 가능 수를 표시한다. 원격 이름은 각 로컬 브랜치의 보조 정보다.
- 기본은 전체 로컬 목록이며, 이름/상태 검색과 stale만 보기로 좁힌다. 사용 중인 브랜치도 숨기지 않는다.
- 각 행은 로컬 이름, stale/원격 존재 상태, worktree 보호 이유, 최신 커밋 순으로 읽힌다.
- 삭제 가능한 stale 행만 선택한다. 그 외 체크박스는 비활성화하고 이유를 label과 tooltip으로 설명한다.
- 주 행동은 선택 항목 삭제 검토이며, 실제 삭제는 기존 이름 확인과 삭제 직전 Git 재검증을 거친다.
- VS Code 공용 표면·글꼴·버튼·포커스 토큰을 재사용한다. 목록 경계는 하나의 얇은 구분선으로 표시한다.
- 좁은 화면에서는 최신 커밋 열을 생략하고 이름·상태·보호 사유와 선택 컨트롤을 유지한다.
- 로컬 브랜치 없음, 검색 결과 없음, 원격 미설정, 삭제 가능한 항목 없음, 선택·취소 상태를 구분한다.
- 실제 Git fixture에서 로컬/원격 동명 비교와 보호 상태를 검사하고, 렌더링 화면에서는 검색·필터·선택 및 390/768/1440px 레이아웃을 별도로 검증한다.
